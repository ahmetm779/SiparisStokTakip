...
namespace SiparisStokTakip.API.Controllers
{
    ...
    public class SiparisStokTakipController : Controller
    {
        ...
        public async Task<IActionResult> CreateSiparisBilgileri([FromBody] Siparis siparis)
        {
            ...
            var fiyat = dbContext.Urunler.FirstOrDefault(urun => urun.UrunID == siparis.UrunID);


            siparis.SiparisTarihi = DateTime.Now;
            siparis.ToplamTutar = fiyat.BirimFiyati;
        ...
        /// <summary>
        /// Bütün Gelir Gider Bilgileri Getirir
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        [Route("[action]")]
        public async Task<IActionResult> GetAllGelirGiderBilgileri()
        {
            var gelirGider = await _siparisStokTakipService.GetAllGelirGiderBilgileri();
            return Ok(gelirGider);
        }

        /// <summary>
        /// ID ye Göre Gelir Gider Getirir
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        [HttpGet]
        [Route("[action]/{id}")]
        public async Task<IActionResult> GetGelirGiderBilgileriById(int id)
        {
            var gelirGider = await _siparisStokTakipService.GetGelirGiderBilgileriById(id);
            if (gelirGider != null)
            {
                return Ok(gelirGider);
            }
        ...
        /// Gelir Gider Ekler
        /// </summary>
        /// <param name="gelirGider"></param>
        /// <returns></returns>
        [HttpPost]
        [Route("[action]")]
        public async Task<IActionResult> CreateGelirGiderBilgileri([FromBody] GelirGider gelirGider)
        {
            var createdGelirGider = await _siparisStokTakipService.CreateGelirGiderBilgileri(gelirGider);
            return CreatedAtAction("Get", new { id = createdGelirGider.ID }, createdGelirGider);
        }

        /// <summary>
        /// Gelir Gider bilgilerini günceller
        /// </summary>
        /// <param name="gelirGider"></param>
        /// <returns></returns>
        [HttpPut]
        [Route("[action]")]
        public async Task<IActionResult> UpdateGelirGiderBilgileri([FromBody] GelirGider gelirGider)
        {
            if (await _siparisStokTakipService.GetGelirGiderBilgileriById(gelirGider.ID) != null)
            {
                return Ok(await _siparisStokTakipService.UpdateGelirGiderBilgileri(gelirGider));
            }
            return NotFound();
        }
        ...
        [HttpDelete]
        [Route("[action]/{id}")]
        public async Task<IActionResult> DeleteGelirGiderBilgileri(int id)
        {
            if (await _siparisStokTakipService.GetGelirGiderBilgileriById(id) != null)
            {
                await _siparisStokTakipService.DeleteGelirGiderBilgileri(id);
                return Ok();
            }
            return NotFound();
        }
